...
aba = st.tabs(["📋 Registrar Ocorrência", "🔍 Consultar", "🛠️ Gerenciar", "📝 Exportar", "Importar Aluno"])
...
with aba[4]:
    ...
    if arquivo is not None:
        try:
            ...
            if "CGM" in df.columns and "Nome do Estudante" in df.columns and "Telefone" in df.columns:
                df = df[["CGM", "Nome do Estudante", "Telefone"]]
                df.columns = ["cgm", "nome", "telefone"]
                ...
                for _, row in df.iterrows():
                    try:
                        ...
                        st.error(f"Erro ao inserir CGM {row['cgm']}: {e}")
                conn.commit()
                st.success(f"{inseridos} alunos importados com sucesso!")
            else:
                st.error("O arquivo não tem o formato esperado. As colunas devem ser: CGM, Nome do Estudante, Telefone.")
        except Exception as e:
